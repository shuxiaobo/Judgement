#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Created by ShaneSue on 21/03/2018
import numpy as np
...
class TextCNNRNN(object):
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        with tf.device('/gpu:0'), tf.name_scope('embedding'):
            ...
            self.embedded_chars = tf.reduce_mean(self.embedded_chars, axis=2)
            emb = tf.expand_dims(self.embedded_chars, -1)  # batch * seq_len * emb_size * 1

        pooled_concat = []
        reduced = np.int32(np.ceil((sequence_length) * 1.0 / max_pool_size))  # 上取整 , 求pool之后的大小
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope('conv-maxpool-%s' % filter_size):
                ...
                emb_pad = tf.concat([pad_prio, emb, pad_post],
                                    1)  # 前后都pad，留作卷积的SAME size用，因为要在第二个维度seq_len做卷积，而不是其他(? , pad_prio + emb + pad_post, emb_size, 1)

                filter_shape = [filter_size, embedding_size, 1, num_filters]
                W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name='W')
                b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name='b')
                conv = tf.nn.conv2d(emb_pad, W, strides=[1, 1, 1, 1], padding='VALID',
                                    name='conv')  # b * seq_len-1/2(filter_size) * embedding_size * num_filter
        ...
        inputs = [tf.squeeze(input_, [1]) for input_ in
                  tf.split(pooled_concat, num_or_size_splits=int(reduced),
                           axis=1)]  # input的组成结构，每一个元素是一个时刻的状态，所以必须拆分，按照时刻状态拆分出来。4 * [? * 96]
        ...
        outputs, state = tf.contrib.rnn.static_rnn(lstm_cell, inputs, initial_state=self._initial_state,
                                                   sequence_length=self.real_len)  # outputs是一个数组，4 * [? * hidden_unit]
        ...
        with tf.variable_scope('Output'):
            ...
            for i in range(1, len(outputs)):
                ...
                mat = tf.matmul(ind, one)  # i < real_len的时候，mat为1，否则为0
        ...
        with tf.name_scope('loss'):
            losses = tf.nn.softmax_cross_entropy_with_logits(labels=self.input_y,
                                                             logits=self.scores,
                                                             name='loss')  # only named arguments accepted
        ...
        tf.summary.scalar('num_correct', self.num_correct)
