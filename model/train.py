...
def init_model(words_dict, feature_dict):
    model = ReaderModel(input_size, hidden_size, output_size, words_dict, TAGS_LEN, CLASS_LEN, feature_dict,
...
def make_dataset(data):
    ...
    data_loader = DataLoader(rnn_dataset, batch_size=32, shuffle=True,
                             num_workers=4, collate_fn=util.collate_batch, pin_memory=False, drop_last=False)
    return data_loader


def main(args):
    data, word2ids = util.load_train_data(util.load_train_data(train_file, word2vec_file))
    feature_dict = util.build_feature_dict(args, data)
    model = init_model(words_dict=word2ids, feature_dict=feature_dict)
